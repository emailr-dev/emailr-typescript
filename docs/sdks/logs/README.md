...
<!-- UsageSnippet language="typescript" operationID="listLogs" method="get" path="/v1/logs" -->
```typescript
...
async function run() {
  const result = await emailr.logs.list({
    eventType: "delivered",
  });
...
import { logsList } from "emailr/funcs/logsList.js";
...
async function run() {
  const res = await logsList(emailr, {
    eventType: "delivered",
  });
  if (res.ok) {
...
**Promise\<[models.EmailEvent[]](../../models/.md)\>**
