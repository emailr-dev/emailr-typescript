...
import { EmailrDefaultError } from "../models/errors/emailrdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | EmailrDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | EmailrDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new EmailrDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new EmailrDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
